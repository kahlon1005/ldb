...
FROM (
	...
	FROM (
		SELECT 
			DISTINCT a.cont, b.whse_code, b.bol_id, a.shipunit_rid, s.wgt scaled_wgt, --c.scaled_wgt,
		...
		FROM bol b
		INNER JOIN shipunit_f s ON b.whse_code = s.whse_code AND b.bol = s.bol
		INNER JOIN shipunit_f_es c ON c.whse_code = s.whse_code AND c.shipunit_rid = s.shipunit_rid  
		INNER JOIN iv_f a ON a.whse_code = s.whse_code AND a.cont = s.cont		
		--and  b.bol_id = 104
	) q
