DROP VIEW IF EXISTS [dbo].[ob_ord_summ_ldb];
GO

CREATE VIEW [dbo].[ob_ord_summ_ldb]
AS
SELECT q3.whse_code,  q3.ob_oid, RIGHT(q3.ob_oid,4) bignum, om_f.wave, om_f.shipment,
	...
	ISNULL(q3.cont,'') cont, q3.liquor_cases_ord, q3.liquor_cases_pckd, q3.liquor_btls_ord, q3.liquor_btls_pckd, q3.non_liquor_pcs_ord, q3.non_liquor_pcs_pckd, 0 scaled_wgt, 	
	om_f.ship_complete,
	...
	IIF(q3.cont IS NULL, 'N', IIF((SELECT COUNT(1) FROM cm_f WHERE cm_f.whse_code = q3.whse_code AND cm_f.to_cont = q3.cont AND cm_f.session_app_id > 0) > 0, 'Y', 'N')) locked_YN, 
	IIF(q3.cont IS NULL, 
		...
		IIF((SELECT COUNT(1) FROM cm_f WHERE cm_f.whse_code = q3.whse_code AND cm_f.to_cont = q3.cont) > 0, 'Y', 'N')) pend_cmd_YN, 		
	IIF(q3.cont IS NULL, 'N', IIF((SELECT COUNT(1) from cm_f WHERE cm_f.whse_code = q3.whse_code AND cm_f.to_cont = q3.cont AND cm_f.cmd_stt = 'SUSP') > 0, 'Y', 'N')) suspended_cmd_YN 
FROM (
	...
	FROM (
		SELECT q1.whse_code, q1.ob_oid, IIF(cn_f.in_cont = ' ', q1.cont, cn_f.in_cont) cont, q1.cont f_cont, q1.sku, q1.pkg, q1.ord_qty, q1.qty, q1.qty1in2, q1.non_liquor,cn_f.cntype,
   		    IIF(q1.non_liquor = 'N', FLOOR(q1.ord_qty/q1.qty1in2), 0) b_liquor_cases_ord,			
			IIF(q1.non_liquor = 'N' AND cn_f.cntype NOT LIKE 'BX%', FLOOR(q1.ord_qty/q1.qty1in2), 0) liquor_cases_ord,
			IIF(q1.non_liquor = 'N' AND cn_f.cntype NOT LIKE 'BX%', FLOOR(q1.qty/q1.qty1in2), 0) liquor_cases_pckd,
			IIF(q1.non_liquor = 'N', FLOOR(q1.ord_qty%q1.qty1in2), 0) b_liquor_btls_ord,			
			IIF(q1.non_liquor = 'N' AND cn_f.cntype LIKE 'BX%', FLOOR(q1.ord_qty), 0) liquor_btls_ord,
			IIF(q1.non_liquor = 'N' AND cn_f.cntype LIKE 'BX%', FLOOR(q1.qty), 0) liquor_btls_pckd,
			IIF(q1.non_liquor = 'Y', FLOOR(q1.ord_qty), 0) non_liquor_pcs_ord,
			IIF(q1.non_liquor = 'Y', FLOOR(q1.qty), 0) non_liquor_pcs_pckd	
		FROM (
			SELECT q.whse_code, q.ob_oid, q.cont, q.sku, q.pkg, q.ord_qty, q.qty, pm_f.qty1in2, 
				 IIF(p.custom_char_01 IS NOT NULL, 'Y', 'N') non_liquor
			FROM (
				SELECT od_f.whse_code, od_f.ob_oid, iv_f.cont, od_f.sku, od_f.pkg, IIF(iv_f.cont IS NULL, od_f.ord_qty,SUM(iv_f.qty)) ord_qty,  IIF(iv_f.mod_id = 'cm_f', 0, SUM(iv_f.qty)) qty 
				FROM od_f
				...
				GROUP BY od_f.whse_code, od_f.ob_oid, iv_f.cont, od_f.sku, od_f.pkg, iv_f.mod_id, od_f.ord_qty
			) q
...
--WHERE q3.ob_oid = '1090933' AND q3.cont = 'C1003123';
