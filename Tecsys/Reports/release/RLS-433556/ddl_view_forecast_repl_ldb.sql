...
FROM 
  (SELECT pm.whse_code, pm.sku, pm.sku_desc, 
    ...
    LEFT JOIN (SELECT iv.whse_code, iv.sku, SUM(qty) AS recv_iv FROM iv_f iv INNER JOIN lc_f lc ON iv.loc=lc.loc 
	  WHERE lc.loc_type='RECV' GROUP BY iv.whse_code, iv.sku) iv1 ON pm.whse_code=iv1.whse_code AND pm.sku=iv1.sku
    LEFT JOIN (SELECT iv.whse_code, iv.sku, SUM(qty) AS fifo_iv FROM iv_f iv INNER JOIN lc_f lc ON iv.loc=lc.loc 
