-- Outbound Summary Archive 
CREATE VIEW [dbo].[v_ob_ord_stt_summ_a_ldb]
AS
SELECT
    v_u_om_f.om_rid,
	v_u_om_f.whse_code, 
	...
	v_u_om_f.ob_oid,
	cust_order = STUFF(
			(SELECT DISTINCT ', ' + cust_oid FROM v_u_od_a v_u_od_f
	...
	v_u_om_f.wave
FROM v_u_om_a v_u_om_f
full outer join v_u_bol_po bol_po on v_u_om_f.ob_oid = bol_po.ship_po and v_u_om_f.whse_code = bol_po.whse_code
full outer join v_u_bol bol on bol_po.bol_id = bol.bol_id and bol_po.whse_code = bol.whse_code
full outer join v_u_od_a v_u_od_f on v_u_om_f.ob_oid = v_u_od_f.ob_oid and v_u_om_f.whse_code = v_u_od_f.whse_code
