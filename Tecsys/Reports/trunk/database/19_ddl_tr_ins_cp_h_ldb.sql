...
BEGIN
	DECLARE 
		...
		@to_cont nvarchar(40),
		@final_loc char(1),
		@task_code nvarchar(4)
		
	
	SELECT @ob_oid = ob_oid, @whse_code = whse_code, @task_code = task_code, @final_loc = final_loc, @to_cont = to_cont FROM INSERTED;
	
	IF @task_code = 'PICK' AND NOT EXISTS (SELECT 1 FROM cm_f WHERE whse_code = @whse_code AND ob_oid = @ob_oid)
	BEGIN	
