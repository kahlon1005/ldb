DROP VIEW IF EXISTS [dbo].[bol_po_ldb]
GO

CREATE VIEW [dbo].[bol_po_ldb]
AS
...
FROM ( 
SELECT q2.whse_code, q2.bol_id, q2.bol_po_seq, q2.ob_oid, q2.cont, 
...
FROM (
SELECT q1.whse_code, q1.bol_id, q1.bol_po_seq, q1.ob_oid, IIF(cn_f.in_cont = ' ', q1.cont, cn_f.in_cont) cont
...
FROM (
SELECT q.whse_code, q.bol_id, q.bol_po_seq, q.ob_oid, q.cont, q.cntype,
		q.sku, q.sku_desc,
...
FROM (
	SELECT bol_po.whse_code, bol_po.bol_id, bol_po.bol_po_seq, iv_f.ob_oid, cn_f.cont, cn_f.cntype,
		    ...
		    SUM(iv_f.qty) qty
	FROM bol_po
		INNER JOIN v_u_iv_f_ldb iv_f ON bol_po.whse_code = iv_f.whse_code AND iv_f.ob_oid = bol_po.ship_po
		INNER JOIN v_u_cn_f_ldb cn_f ON cn_f.whse_code = iv_f.whse_code AND cn_f.cont = iv_f.cont
		INNER JOIN pm_f ON pm_f.whse_code = iv_f.whse_code AND pm_f.sku = iv_f.sku AND pm_f.pkg = iv_f.pkg
		LEFT JOIN wms_system_properties_ldb prop ON prop.whse_code = pm_f.whse_code AND pm_f.custom_char_6 = prop.custom_char_01 AND prop.property_name = 'NON-LIQUOR'	
--WHERE iv_f.ob_oid = '828526'
	GROUP BY bol_po.whse_code, bol_po.bol_id, bol_po.bol_po_seq, iv_f.ob_oid, cn_f.cont, cn_f.cntype,
		    pm_f.sku, pm_f.sku_desc, IIF(prop.custom_char_01 IS NULL,'N','Y'), pm_f.wgt1, ISNULL(pm_f.custom_numeric_3,1)
) q 
GROUP BY q.whse_code, q.bol_id, q.bol_po_seq, q.ob_oid,  q.cont, q.cntype,q.sku, q.sku_desc
) q1
INNER JOIN v_u_cn_f_ldb cn_f  ON cn_f.whse_code = q1.whse_code AND cn_f.cont = q1.cont
GROUP BY q1.whse_code, q1.bol_id, q1.bol_po_seq, q1.ob_oid, IIF(cn_f.in_cont = ' ', q1.cont, cn_f.in_cont)
) q2
INNER JOIN v_u_cn_f_ldb cn_f ON cn_f.whse_code = q2.whse_code AND cn_f.cont = q2.cont
