...
FROM (
	SELECT q3.whse_code,  q3.ob_oid, om_f.ob_ord_stt, RIGHT(q3.ob_oid,4) bignum, om_f.wave, om_f.shipment,
		...
		IIF(q3.cont IS NULL, 
			...
			IIF((SELECT COUNT(1) FROM cm_f WHERE cm_f.whse_code = q3.whse_code AND (cm_f.to_cont = q3.cont OR cm_f.to_cont = replace(q3.cont,'C','SP') OR cm_f.cont = q3.cont)) > 0, 'Y', 'N')) pend_cmd_YN, 					
